...
/**
 * Fallback charging stations for common routes (mock data)
 */
const fallbackChargingStations = [
  {
    ...
    AddressInfo: {
      ID: 1,
      Title: "Sample Fast Charging Station",
      AddressLine1: "Highway Service Area",
      Town: "Sample City",
    ...
    Connections: [{ PowerKW: 50, ConnectionType: { Title: "CCS" } }],
    distanceFromRoute: 5
  }
];
...
export async function calculateTripPlan(tripData) {
  try {
    ...
    const usingDemoData = nearbyChargers.length > 0 && nearbyChargers[0].ID?.toString().startsWith('mock_');
